...
const DEFAULT_MAX_BOTS = Number(process.env.JADIBOT_MAX_BOTS || 3);
...
class JadibotManager {
  ...
  readDB() {
    const data = readJson(DATA_FILE, { sessions: {} });
  ...
  upsertRecord(phoneNumber, patch = {}) {
    ...
    data.sessions[phone] = {
      phoneNumber: phone,
      sessionDir: this.getSessionDir(phone),
      createdAt: now,
      ...(data.sessions[phone] || {}),
      ...patch,
      updatedAt: now
  ...
  getRecord(phoneNumber) {
    const phone = normalizePhoneNumber(phoneNumber);
    return this.readDB().sessions[phone] || null;
  }

  listRecords() {
    const data = this.readDB();
    return Object.values(data.sessions || {}).map((record) => ({
      ...record,
  ...
  async startSession(options = {}) {
    ...
    if (forceNewPairing && fs.existsSync(sessionDir)) {
  ...
  async stopSession(phoneNumber, options = {}) {
    ...
    if (options.deleteSession) {
      const sessionDir = this.getSessionDir(phone);
      await fs.promises.rm(sessionDir, { recursive: true, force: true });
      this.removeRecord(phone);
    }
